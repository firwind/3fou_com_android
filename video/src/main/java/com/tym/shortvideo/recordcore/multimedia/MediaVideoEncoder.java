...
public class MediaVideoEncoder extends MediaEncoder {
    ...
    public MediaVideoEncoder(final MediaMuxerWrapper muxer, final MediaEncoderListener listener,
        ...
        if (DEBUG) {
            LogUtils.i(TAG, "MediaVideoEncoder: ");
        }
        mWidth = width;
        mHeight = height;
    }
    ...
    public MediaVideoEncoder(final MediaMuxerWrapper muxer, final MediaEncoderListener listener,
                             final int width, final int height, int bitRate) {
        super(muxer, listener, true);
        if (DEBUG) {
            LogUtils.i(TAG, "MediaVideoEncoder: ");
        }
    ...
    protected void prepare() throws IOException {
        ...
        if (DEBUG) {
            LogUtils.i(TAG, "selected codec: " + videoCodecInfo.getName());
        }
    ...
    private int calcBitRate() {
        ...
        LogUtils.i(TAG, String.format("bitrate = % 5.2f[Mbps]", bitrate / 1024f / 1024f));
        return bitrate;
    }
    ...
    protected static final MediaCodecInfo selectVideoCodec(final String mimeType) {
        if (DEBUG) {
            LogUtils.d(TAG, "selectVideoCodec:");
        }
        ...
        for (int i = 0; i < numCodecs; i++) {
            ...
            for (int j = 0; j < types.length; j++) {
                if (types[j].equalsIgnoreCase(mimeType)) {
                    if (DEBUG) {
                        LogUtils.i(TAG, "codec:" + codecInfo.getName() + ",MIME=" + types[j]);
                    }
    ...
    protected static final int selectColorFormat(final MediaCodecInfo codecInfo, final String mimeType) {
        if (DEBUG) {
            LogUtils.i(TAG, "selectColorFormat: ");
        }
        int result = 0;
        ...
        if (result == 0) {
            LogUtils.e(TAG, "couldn't find a good color format for " + codecInfo.getName() + " / " + mimeType);
        }
        return result;
    }
    ...
    private static final boolean isRecognizedViewoFormat(final int colorFormat) {
        if (DEBUG) {
            LogUtils.i(TAG, "isRecognizedViewoFormat:colorFormat=" + colorFormat);
        }
